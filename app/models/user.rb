...
class User < ApplicationRecord
  ...
  has_many :reverse_of_user_follows, class_name: 'UserFollow', foreign_key: 'follow_id'
  ...
  def follow(user)
    ...
    # user が自分自身ではない場合は登録
    unless self == user
      self.user_follows.find_or_create_by(follow_id: user.id)
    end
  end
  ...
  def unfollow(user)
    ...
    user_follows.destroy if user_follows
  end
